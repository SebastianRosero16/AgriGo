...
export const EmailVerification: React.FC<EmailVerificationProps> = ({
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      if (response.success) {
        // If backend returns the code (no SMTP configured), show it
        if ((response as any).devCode) {
          const dc = (response as any).devCode as string;
          setDevCode(dc);
          setCode(dc); // pre-fill the input
          toast.info(`📋 Tu código de verificación es: ${dc}`, { autoClose: false });
        } else {
  ...
  return (
    <Card>
      ...
      {/* Dev mode: show code directly when no email is configured */}
      {devCode && (
        <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-4 mb-4 text-center">
          <p className="text-xs font-semibold text-yellow-700 mb-1">
            ⚠️ Servidor sin email configurado
          </p>
          <p className="text-sm text-yellow-800">Tu código de verificación es:</p>
          <p className="text-3xl font-mono font-bold text-yellow-900 tracking-widest mt-1">
            {devCode}
          </p>
          <p className="text-xs text-yellow-600 mt-1">Ya está copiado en el campo de abajo</p>
        </div>
      )}
